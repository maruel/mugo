...
// goc transpiles a subset of the Go language to C++.
//
...
//    temporary struct
//  - string is converted to const char *
...
// nodeType returns the string representation of the type of a node, in Go
// language.
...
func identName(x ast.Expr) string {
	...
	case *ast.SelectorExpr:
		return identName(t.X) + "." + identName(t.Sel)
	case *ast.Ident:
		return t.Name
	default:
		return "<N/A>"
...
func fieldToType(f *ast.Field) (string, bool) {
	...
	case *ast.ArrayType:
		return "[]" + nodeType(arg.Elt), false
	case *ast.Ellipsis:
		return nodeType(arg.Elt), true
	case *ast.FuncType:
	...
	case *ast.SelectorExpr:
		// TODO(maruel): Test. fmt.Printf("  - Selector Arg: %#v\n", arg)
		return arg.Sel.Name, false
	case *ast.StarExpr:
		return "*" + arg.X.(*ast.Ident).Name, false
	default:
		panic(fmt.Errorf("unexpected param type: %# v", pretty.Formatter(f)))
	}
}
...
func handleFuncDecl(out *output, fd *ast.FuncDecl) error {
	...
	if fd.Type.Results != nil {
		...
		ret = nodeType(fd.Type.Results.List[0].Type)
	}
	params, extra := extractArgumentsType(fd)
	if extra {
		return out.Errorf(fd, "unsupported param type: %# v", pretty.Formatter(fd.Type.Params))
	}
...
func handleCallExpr(out *output, c *ast.CallExpr) error {
	...
	out.Writef(c, "%s(%s)", identName(c.Fun), strings.Join(args, ", "))
	return nil
}
